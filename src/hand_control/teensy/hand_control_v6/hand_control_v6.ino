// =============================================================================
// hand_control_v6.ino — Robotic Hand Controller
...
//   2  | F3     | 41 (A17)  | active
//   3  | F4     | 40 (A16)  | active
//   4  | F5     | — (TBD)   | PENDING — not yet wired
//
...
//   INA_STREAM  hz=N
//   INA_STREAM_STOP
//   FSR         ch=N label=L finger=F raw=R scaled=S pct=P pressed=0|1
//   FSR_STREAM  hz=N
//   FSR_STREAM_STOP
...
//   STATUS_FSR     ch=N label=L finger=F pin=P scaled=S pressed=0|1 pending=0|1
//   STATUS_SYSTEM  estop=0|1 speed_dps=D ina_stream=0|1 fsr_stream=0|1
//   ERR         msg=<text>
...
const float INIT_ANGLE[NUM_CHANNELS] = {
  ...
  45.0f,   // 10  thumb    rotation
   0.0f,   // 11  thumb    1st curvature (monitored)
   0.0f,   // 12  index    1st curvature (monitored)
   0.0f,   // 13  middle   1st curvature (monitored)
   0.0f,   // 14  ring     1st curvature (monitored)
   0.0f,   // 15  pinky    1st curvature (monitored)
};

const float JOINT_MIN[NUM_CHANNELS] = {
  0.0f,   //  0  unused   — locked at 90
  0.0f,   //  1  unused   — locked at 90
  0.0f,   //  2  pinky    2nd curvature
...
const float JOINT_MAX[NUM_CHANNELS] = {
   90.0f,  //  0  unused   — locked at 90
   90.0f,  //  1  unused   — locked at 90
  ...
  180.0f,  // 11  thumb    1st curvature (monitored)
  180.0f,  // 12  index    1st curvature (monitored)
  180.0f,  // 13  middle   1st curvature (monitored)
  180.0f,  // 14  ring     1st curvature (monitored)
  180.0f,  // 15  pinky    1st curvature (monitored)
};
...
bool  homingAll   = false;   // true while HOME (all) is in progress
int   homeRemaining = 0;      // channels not yet at init position
...
int      seqHead    = 0;     // index of next entry to dispatch
int      seqTail    = 0;     // index of next free slot
int      seqActive  = -1;    // channel currently executing (-1 = idle)
bool     seqRunning = false;
float    seqAngle   = 0.0f;  // the angle that was commanded (for SEQ_DONE)
char     seqCmd[16] = "";    // "CURVATURE" or "ROTATE" (for SEQ_DONE)
...
// FSR 0 — pin 15 (A1) — finger 1
#define FSR0_PIN        15
#define FSR0_MIN        2
#define FSR0_MAX        20
#define FSR0_THRESHOLD  1

// FSR 1 — pin 14 (A0) — finger 2
#define FSR1_PIN        14
#define FSR1_MIN        5
#define FSR1_MAX        600
#define FSR1_THRESHOLD  50

// FSR 2 — pin 41 (A17) — finger 3
#define FSR2_PIN        41
#define FSR2_MIN        5
#define FSR2_MAX        100
#define FSR2_THRESHOLD  1

// FSR 3 — pin 40 (A16) — finger 4
#define FSR3_PIN        40
#define FSR3_MIN        5
#define FSR3_MAX        3800
#define FSR3_THRESHOLD  200

// FSR 4 — pin TBD — finger 5  ← PENDING: assign pin and calibrate before enabling
...
// Maps FSR index to finger name (for STATUS output)
...
void setup() {
  ...
  for (int i = 0; i < NUM_FSR; i++) {
    if (i == 4 && FSR4_PENDING) continue;   // skip unconnected FSR4
  ...
  if (Wire.endTransmission() != 0) {
    Serial.print("ERR msg="); Serial.print("msg="); Serial.println("PCA9685 not found on Wire at 0x40 — check wiring");
    while (1);
  }
  pca.begin();
  ...
  // ── Validate init angles ──
  for (int ch = 0; ch < NUM_CHANNELS; ch++) {
    if (INIT_ANGLE[ch] < JOINT_MIN[ch] || INIT_ANGLE[ch] > JOINT_MAX[ch]) {
      Serial.print("ERR INIT_ANGLE["); Serial.print(ch);
      Serial.print("] = "); Serial.print(INIT_ANGLE[ch], 1);
      Serial.print(" outside ["); Serial.print(JOINT_MIN[ch], 1);
...
void handleSerial() {
  ...
  String cmd = "";
  String args[14];  // extended for MOVESET (up to 14 ch:angle pairs)
  int    argc = 0;
  bool   gotCmd = false;
  int    start  = 0;
  ...
  // ── MOVE ───────────────────────────────────────────────────────────────────
  if (cmd == "MOVE") {
    if (argc < 2) { Serial.print("ERR msg="); Serial.print("msg="); Serial.println("MOVE requires channel and angle"); return; }
    int   ch  = args[0].toInt();
    float ang = args[1].toFloat();
    if (ch < 0 || ch >= NUM_CHANNELS) { Serial.print("ERR msg="); Serial.print("invalid channel "); Serial.println(ch); return; }
    if (ch == 0 || ch == 1)           { Serial.print("ERR msg="); Serial.print("msg="); Serial.println("CH0/1 unused — use 2–15"); return; }
    if (ang < 0 || ang > 180)         { Serial.print("ERR msg="); Serial.print("invalid angle "); Serial.println(ang,1); return; }
    if (globalEstop)                  { Serial.print("ERR msg="); Serial.print("msg="); Serial.println("in ESTOP — send RESUME_ALL first"); return; }
    float clamped = constrain(ang, JOINT_MIN[ch], JOINT_MAX[ch]);
    if (clamped != ang) {
      Serial.print("INFO ch="); Serial.print(ch);
      Serial.print(" clamped "); Serial.print(ang,1); Serial.print(" → "); Serial.println(clamped,1);
      ang = clamped;
    }
  ...
  // ── STOP ───────────────────────────────────────────────────────────────────
    ...
    if (argc < 1) { Serial.print("ERR msg="); Serial.print("msg="); Serial.println("STOP needs channel. Use INOSTOP/FSRSTOP for streams"); return; }
    int ch = args[0].toInt();
    if (ch < 0 || ch >= NUM_CHANNELS) { Serial.print("ERR msg="); Serial.print("invalid channel "); Serial.println(ch); return; }
    sv[ch].stopped     = true;
  ...
  // ── ESTOP ──────────────────────────────────────────────────────────────────
    ...
    globalEstop = true;
    Serial.print("ESTOP angle=");
    for (int ch = 0; ch < NUM_CHANNELS; ch++) {
      sv[ch].stopped = true; sv[ch].targetAngle = sv[ch].currentAngle; disableChannel(ch);
      if (ch >= 2) {
        Serial.print("ch"); Serial.print(ch); Serial.print("="); Serial.print(sv[ch].currentAngle,1);
  ...
  // ── RESUME ─────────────────────────────────────────────────────────────────
    ...
    if (argc < 1) { Serial.print("ERR msg="); Serial.print("msg="); Serial.println("RESUME needs channel"); return; }
    int ch = args[0].toInt();
    if (ch < 0 || ch >= NUM_CHANNELS) { Serial.print("ERR msg="); Serial.print("invalid channel "); Serial.println(ch); return; }
    if (ch == 0 || ch == 1) { Serial.print("ERR msg="); Serial.print("msg="); Serial.println("CH0/1 unused"); return; }
  ...
  // ── CURVATURE <angle> ─────────────────────────────────────────────────────
  // Moves all curvature joints one by one: thumb→index→middle→ring→pinky
  // Each finger does curve1 first, then curve2 (thumb has no curve2).
    ...
    // Build queue: thumb curve1, index curve1+curve2, middle curve1+curve2,
    //              ring curve1+curve2, pinky curve1+curve2
    //  ch order:  11, 12, 8, 13, 6, 14, 4, 15, 2
  ...
  // ── ROTATE <angle> ─────────────────────────────────────────────────────────
  // Moves all rotation joints one by one: thumb→index→middle→ring→pinky
  //  ch order: 10, 9, 7, 5, 3
  else if (cmd == "ROTATE") {
  ...
  // All curvature joints start moving simultaneously.
  else if (cmd == "CURVATURE_SYNC") {
    if (argc < 1) { Serial.println("ERR msg=CURVATURE_SYNC_requires_angle"); return; }
    if (globalEstop) { Serial.println("ERR msg=in_ESTOP_send_RESUME_ALL_first"); return; }
    float ang = args[0].toFloat();
    if (ang < 0 || ang > 180) { Serial.println("ERR msg=angle_must_be_0_to_180"); return; }
    // Curvature channels: 11(thumb), 12(index), 8(index2), 13(middle), 6(middle2),
    //                     14(ring), 4(ring2), 15(pinky), 2(pinky2)
  ...
  // ── ROTATE_SYNC <angle> ───────────────────────────────────────────────────
  // All rotation joints start moving simultaneously.
  ...
  // ── MOVEALL <angle> ───────────────────────────────────────────────────────
  // All active joints (curvature + rotation) simultaneously.
  else if (cmd == "MOVEALL") {
  ...
  // Set each listed channel to its own angle, all starting simultaneously.
  // Format: MOVESET 11:90 12:45 8:30 13:60
  // Up to 14 channel:angle pairs per command.
  // Individual MOVE_START lines are suppressed; a single MOVESET_START is sent.
  // Individual MOVE_DONE lines are still sent as each servo arrives.
  else if (cmd == "MOVESET") {
    if (argc < 1) { Serial.println("ERR msg=MOVESET_requires_at_least_one_ch:angle_pair"); return; }
    if (globalEstop) { Serial.println("ERR msg=in_ESTOP_send_RESUME_ALL_first"); return; }
    int applied = 0;
    for (int i = 0; i < argc && i < 4; i++) {
      // Each arg is "ch:angle"
  ...
  // ── SPEED ──────────────────────────────────────────────────────────────────
    ...
    if (argc < 1) { Serial.print("ERR msg="); Serial.print("msg="); Serial.println("SPEED needs value"); return; }
    float dps = args[0].toFloat();
    if (dps < MIN_DPS || dps > MAX_DPS) {
      Serial.print("ERR speed out of range ("); Serial.print(MIN_DPS,0);
      Serial.print("–"); Serial.print(MAX_DPS,0); Serial.println(")"); return;
    }
    globalDPS = dps;
  ...
  // ── STREAM <hz> (INA) ──────────────────────────────────────────────────────
    ...
    if (hz < 1 || hz > 100) { Serial.print("ERR msg="); Serial.print("msg="); Serial.println("STREAM rate 1–100 Hz"); return; }
  ...
  // ── FSRSTREAM <hz> ─────────────────────────────────────────────────────────
    ...
    uint32_t hz = (argc > 0) ? (uint32_t)args[0].toInt() : 10;
    if (hz < 1 || hz > 50) { Serial.print("ERR msg="); Serial.print("msg="); Serial.println("FSRSTREAM rate 1–50 Hz"); return; }
  ...
  // ── FSRFORMAT ──────────────────────────────────────────────────────────────
    ...
    if (argc < 1) { Serial.print("ERR msg="); Serial.print("msg="); Serial.println("FSRFORMAT needs PCT or SCALE"); return; }
    args[0].toUpperCase();
    ...
    else { Serial.print("ERR msg="); Serial.print("msg="); Serial.println("use PCT or SCALE"); }
  }
  ...
  // ── STATUS ─────────────────────────────────────────────────────────────────
    ...
    // One STATUS_SERVO line per channel
    for (int ch = 0; ch < NUM_CHANNELS; ch++) {
      ...
      Serial.print(" ina_ch=");  Serial.println(ic);   // -1 if unmonitored
    }
    // One STATUS_INA line per sensor
    for (int i = 0; i < NUM_SENSORS; i++) {
    ...
    // One STATUS_FSR line per sensor
    for (int i = 0; i < NUM_FSR; i++) {
    ...
    // One STATUS_SYSTEM summary line
  ...
  // ── HELP ───────────────────────────────────────────────────────────────────
    ...
    Serial.print("ERR msg="); Serial.print("unknown: "); Serial.println(cmd);
...
// Start a gradual move on one channel — used by sync commands that
// kick off multiple channels at once. Does NOT send MOVE_START individually;
// caller sends a single SYNC_START instead.
...
// Dispatch the next entry in seqQueue as a normal gradual MOVE.
void dispatchNextSeq() {
  while (seqHead != seqTail) {
    ...
    // Skip if already at target (counts as done immediately)
    ...
    Serial.print(" from=");         Serial.print(sv[ch].currentAngle, 1);
    Serial.print(" to=");           Serial.println(ang, 1);
    return;   // wait for this move to finish
  }
  // Queue empty
  seqActive  = -1;
  seqRunning = false;
...
// Call from loop() — advances queue when active channel finishes.
void stepSeqQueue() {
  ...
  for (int ch = 0; ch < NUM_CHANNELS; ch++) {
    ...
    if (fabsf(diff) < ARRIVE_THRESH) {
      if (!sv[ch].moveDoneSent) {
        ...
        Serial.print(" angle=");       Serial.println(sv[ch].currentAngle, 1);
        sv[ch].moveDoneSent = true;
        // Check if this completes a HOME_ALL
        if (homingAll) {
          homeRemaining--;
          if (homeRemaining <= 0) {
            homingAll = false;
...
void readAllFSR(bool rawOnly) {
  for (int i = 0; i < NUM_FSR; i++) {
    ...
    bool pending_r = (i == 4 && FSR4_PENDING);
    Serial.print("FSR");
    Serial.print(" ch=");      Serial.print(i);
    Serial.print(" label=");   Serial.print(FSR_LABEL[i]);
    Serial.print(" finger=");  Serial.print(FSR_FINGER[i]);
    Serial.print(" pending="); Serial.print(pending_r ? 1 : 0);

    if (pending_r) { Serial.println(); continue; }

    int  rawVal_r = analogRead(FSR_PIN[i]);
    int  scaled_r = scaleFSR(rawVal_r, i);
    bool pressed_r = (scaled_r >= FSR_THRESH[i]);

    Serial.print(" raw=");     Serial.print(rawVal_r);
    Serial.print(" scaled=");  Serial.print(scaled_r);
    Serial.print(" pct=");     Serial.print(scaled_r / 10.0f, 1);
    Serial.print(" thresh=");  Serial.print(FSR_THRESH[i]);
    Serial.print(" pressed="); Serial.println(pressed_r ? 1 : 0);
  }
}
...
void printHelp() {
  Serial.println("INFO ============================================================");
  Serial.println("INFO  hand_control_v4");
  ...
  Serial.println("INFO    MOVESET ch:a [ch:a..] named channels simultaneously");
  ...
  Serial.println("INFO  SEQ_START/SEQ_DONE reported for CURVATURE and ROTATE");
